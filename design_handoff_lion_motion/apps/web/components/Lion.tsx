...
const MOOD: Record<string, MoodCfg> = {
  ...
  notify: { base: "scene-notification", alt: "Sư tử rung chuông nhắc em" },
  ...
  rebel: {
    base: "turn-front",
    alt: "Sư tử dỗi — thầy hỏi, em nghĩ",
    overlays: [
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        el.style.setProperty("--lion-lx", `${((dx / d) * k).toFixed(2)}px`);
        el.style.setProperty("--lion-ly", `${((dy / d) * k).toFixed(2)}px`);
      });
    };
